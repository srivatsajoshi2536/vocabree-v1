...
class LessonService {
  ...
  /**
   * Get English translation for a native script word
   * Searches through all vocabulary to find the English equivalent
   */
  getEnglishTranslation(nativeWord, languageId, skillId = null, level = null) {
    // If the word is already in English (ASCII only), return as is
    if (/^[\x00-\x7F]*$/.test(nativeWord)) {
    ...
    if (skillId && level) {
      try {
        ...
        for (let i = 1; i <= 5; i++) {
          ...
          if (vocab && vocab[wordKey] === nativeWord) {
            return englishVocab && englishVocab[wordKey] ? englishVocab[wordKey] : null;
          }
        }
      } catch (e) {
        // Continue to search all vocabularies
      }
    }
      ...
      for (let lvl = 1; lvl <= 5; lvl++) {
        try {
          ...
          if (vocab && englishVocab) {
            for (let i = 1; i <= 5; i++) {
              const wordKey = `word${i}`;
              if (vocab[wordKey] === nativeWord) {
                return englishVocab[wordKey] || null;
              }
            }
          }
        } catch (e) {
