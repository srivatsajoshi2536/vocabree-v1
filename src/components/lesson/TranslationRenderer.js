...
const TranslationRenderer = ({ exercise, onAnswer, languageId: propLanguageId, isPractice = false, skillId = null, level = null }) => {
  ...
  // Get English translation for the question text (only in lessons, not practice)
  let questionDisplayText = exercise.questionText;
  if (!isPractice && exercise.questionText) {
    const isNativeScript = /[^\x00-\x7F]/.test(exercise.questionText);
    if (isNativeScript) {
      ...
      if (englishTranslation) {
        questionDisplayText = `${exercise.questionText} (${englishTranslation})`;
      }
    }
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.questionText}>{questionDisplayText}</Text>
